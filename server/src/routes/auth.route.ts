 ...
 */
// Stricter limit on auth endpoints: 5 requests/min/IP.
const authRateLimit = { config: routeRateLimit(5, '1 minute') };

export async function authRoutes(app: FastifyInstance): Promise<void> {
  app.post('/auth/request-otp', authRateLimit, async (request, reply) => {
    const { phone } = requestOtpBody.parse(request.body);
    return reply.send(await requestOtp(phone));
  });

  app.post('/auth/verify-otp', authRateLimit, async (request, reply) => {
